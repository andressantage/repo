...
const getUserAll = async()=>{
    ...
    let res = await ( await fetch("http://localhost:4001/embarque",config)).json();//await es para esperar que algo suceda
...
const postUser = async(data)=>{
    ...
    let res = await ( await fetch("http://localhost:4001/embarque",config)).json();
    console.log(res);
}
const putUser = async(data)=>{
    config.method = "PUT";
    config.body = JSON.stringify(data);
    let res = await ( await fetch(`http://localhost:4001/embarque/${data.id}`,config)).json();
    console.log(res);
}
...
const searchUser = async(data)=>{
    config.method = "GET";
    let res = await ( await fetch(`http://localhost:4001/embarque?q=${Object.values(data).join("")}`,config)).json();
    console.log(res);
}