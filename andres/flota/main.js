...
const getUserAll = async()=>{
    ...
    let res = await ( await fetch("http://localhost:4001/flota",config)).json();//await es para esperar que algo suceda
...
const searchUser = async(data)=>{
    config.method = "GET";
    let res = await ( await fetch(`http://localhost:4001/flota?q=${Object.values(data).join("")}`,config)).json();
    console.log(res);
}